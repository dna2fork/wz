...
wz_read_chars(wz_uint8_t ** ret_bytes, wz_uint32_t * ret_len,
  ...
  if (type != WZ_LV0_NAME) {
    ...
    if (inplace == UNK) {
      wz_error("Unsupported string type: 0x%02"WZ_PRIx32"\n",
                      (wz_uint32_t) fmt);
      return ret; }
    if (!inplace) {
      wz_uint32_t offset;
...
wz_deduce_key(wz_uint8_t * ret_key, wz_uint8_t * bytes, wz_uint32_t len,
  ...
  wz_error("Cannot deduce the string key\n");
  return 1;
}
...
wz_decode_bitmap(wz_uint32_t * written, wz_uint8_t * out, wz_uint8_t * in,
  ...
  while (src.u8 < src_end) {
    ...
    wz_uint32_t i;
    if (len > WZ_KEY_ASCII_MAX_LEN) {
      wz_error("Image chunk size is too large: %"WZ_PRIu32"\n", len);
      return 1; }
    for (i = 0; i < len; i++)
...
wz_next_tok(const char ** begin, const char ** end, const char * str,
  ...
  for (;;) {
    const char c = * str;
    if (c == '\0') { * begin = NULL; * end = NULL; return (size_t) 0; }
    if (c != delim) break;
    str++;
  }
  * begin = str++;
  for (;;) {
    const char c = * str;
    if (c == '\0') { * end = str; return (size_t) (str - * begin); }
    if (c == delim) { * end = str + 1; return (size_t) (str - * begin); }
    str++;
  }
}
